 ...
 * all information given by users (manager and clients) between sessions.
 * In a real application this package would contain the interface to the 
 * database. In our case we will simply write the information into text files.
 * 
 * Domain classes (Client, Order, Dish) must provide two methods that are used 
 * in this package:
 * - void toString() that produces a string with all relevant information
